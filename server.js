...
app.post('/api/payment', async (req, res) => {
  ...
  const pkgName    = pkg === 'premium' ? 'Paquete Premium' : pkg === 'starter' ? 'Paquete Esencial' : 'Paquete Basico';
  ...
  const originalAmt  = pkg === 'premium' ? 5000 : pkg === 'starter' ? 2000 : 3000;
  ...
  try {
    ...
    if (!isFreeOrder && finalAmount > 0) {
      if (![2000, 3000, 5000].includes(Number(amountCents))) {
